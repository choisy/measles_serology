...
```{r}
convert_date <- function(x) x |> as.numeric() |> as_date("1899-12-30")
```
...
```{r}
as_date2 <- function(...) lubridate::as_date(..., origin = "1970-01-01")
```
...
**TO DO:** maybe simply use a hash table after conversion as done for the South.
...
```{r}
translation <- c("Benh vien"     = "hospital",  
...
Hash table for the `sex` variable:

```{r}
hash_sex <- rep(c("female", "male"), each = 3) |> 
  setNames(c("f", "F", "Nu", "M", "N", "Nam"))
```

A function that translates the names of the variable of a data frame from Vietnamese to
English using the `translation` hash table:

```{r}
convert_var_names <- function(df) {
  names(df) <- translation[names(df)]
  df
}
```
...
titers <- map(c("IgG Serion 2025 Measles 3021 mau.xls",
  ...
  rename(titer      = `Nong do khang the`,
         conclusion = `Ket qua`) |> 
  mutate(across(conclusion, ~ hash[convert(.x)])) |> 
  unique()
```
...
```{r warning = FALSE}
dataset_north <- c("02FL Tracking sample round 1 to 6 Final.xlsx",
  ...
  mutate(across(province, convert),
         ...
         across(reception_date, ~ as_date2(ifelse(.x == "30/10/2020", ymd(20201030),
                                                  convert_date(.x)))),
         sex       = hash_sex[sex],
...
```{r}
map_int(dataset_north, ~ .x |> is.na() |> sum())
```
...
dataset_all |> 
  ...
  tally() |> 
  ungroup() |> 
  mutate(y = max(hz_lines) + 1 - as.numeric(factor(province, nb_samples2$province))) |> 
...
axis(1, ymd(paste(lb, "0101")), lb)
abline3(h = hz_lines)
text2(rep(ymd(20210701), length(hz_lines)), rev(hz_lines), nb_samples2$province)
...
```{r fig-T1T2T3}
seroprev3timepoints(dataset)
```

```{r fig-T1T2}
seroprevT1T2(dataset)
```
