...
image_titer_density_age <- function(data, xaxis = TRUE, xlab = "age (years)", n = 512,
  ...
  data |> 
    ...
    image(1:70 - .5, seq(min_titer, max_titer, le = n), z = _, axes = FALSE,
          ...
          col = hcl.colors(n, "YlOrRd", rev = TRUE))
...
```{r include=FALSE, eval=FALSE}
estimate_prop2 <- function(df, th) {
  df |> 
    group_by(titer > th) |> 
    tally() |> 
    with(binom.test(n[2], sum(n))) |> 
    extract(c("estimate", "conf.int")) |>
    unlist() |> 
    setNames(c("p", "l", "u"))
}
...
```{r include=FALSE, eval=FALSE}
prepare_data2 <- function(df, th,
                         template = tibble(age_group = levels(dataset$age_group))) {
  tibble(age_group = levels(droplevels(df$age_group))) |> 
    bind_cols(df |> 
                group_by(age_group) |> 
                group_split() |> 
                map_dfr(estimate_prop2, th = 200)) |> 
    left_join(template, y = _, "age_group") |> 
...
dataset |> 
  ...
  with(table(age_group, titer > 200)) |> 
  as.data.frame() |> 
    pivot_wider(names_from = Var2, values_from = Freq) |> 
    mutate(tmp = map2(`TRUE`, `TRUE` + `FALSE`, binom_test2) |> 
             map(extract, c("estimate", "conf.int")) |>
...
```{r}
make_p_age_profile2 <- function(xs = c(.5:19.5, seq(22.5, 67.5, 5), 75)) {
  function(x) {
    plot(xs, rep(.5, length(xs)), ylim = 0:1, type = "n", xaxs = "i", yaxs = "i",
         xlim = c(0, 80), xlab = "age classe (years)", ylab = "sero-prevalence")
    abline2(v = seq(0, 70, 5))
...
p_age_profile2 <- make_p_age_profile2()
```

```{r include=FALSE, eval=FALSE}
plot_age_profile2 <- function(df, th,
                             tmplt = tibble(age_group = levels(dataset$age_group))) {
  df |> 
    prepare_data2(th, tmplt) |> 
    p_age_profile2()
}
```
...
dataset |> 
  group_by(time_point) |> 
  group_walk(~ plot_age_profile2(.x, 200))
par(opar)
```
...
dataset |> 
  ...
  as.data.frame() |> 
  pivot_wider(names_from = Var2, values_from = Freq) |> 
  mutate(tmp = map2(`TRUE`, `TRUE` + `FALSE`, binom.test) |> 
           map(extract, c("estimate", "conf.int")) |>
           map(unlist) |> 
           map(setNames, c("p", "l", "u"))) |> 
  unnest_wider(tmp)
...
where `pred` is an output of the `predict2()` function and a 4-column data frame with
...
where `pred` is an output of the `predict2()` function. A wrapper around the
`predict2()`, `plot_pred()` and `add_ribbon()` functions:
...
Let's try 10 smooting basis to see whether one stands out as much better that the
others:
