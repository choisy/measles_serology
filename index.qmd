...
image_titer_density_age <- function(data, xaxis = TRUE, xlab = "age (years)", n = 512,
                                    ncol = n, min_titer = NULL, max_titer = NULL,
                                    buff = TRUE) {
  ...
  data |> 
    ...
    image(1:70 - .5, seq(min_titer, max_titer, le = n), z = _, axes = FALSE,
          ...
          col = hcl.colors(ncol, "YlOrRd", rev = TRUE))
...
```{r fig.height = 2 * eps * figh}
opar <- par(plt = c(.105, .97, .5, 1 - (1 - .95) / 2 - .15 / 2))
dataset |> 
  mutate(across(titer, log10)) |> 
  image_titer_density_age(xaxis = FALSE, xlab = NA, ncol = 12)
add_years_on_top()
par(plt = c(.105, .97, .15 / 2 + (1 - .95) / 2, .5), new = TRUE)
plot_seroprevalences(dataset)
par(opar)
...
* do the image function but with considering quantiles instead (i.e. multimodal one)
