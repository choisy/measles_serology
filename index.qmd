...
```{r}
south <- mutate(south, across(site, ~ ifelse(is.na(.x), hash[province], .x)))
```
...
abline3(h = hz_lines)
text2(rep(ymd(20210701), length(hz_lines)), rev(hz_lines), unique(south$province))
...
```{r fig-cum_nb_sample}
plot2(NA, xlim = range(south$collection_date), ylim = c(0, 600), xlab = NA,
