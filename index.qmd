...
```{r}
gam2 <- function(formula, data, ...) {
  mgcv::gam(formula, data, family = binomial, method = "REML", ...)
}
```
...
plot_pred <- function(pred, ylim = NULL, col = 4, ...) {
  ...
  with(pred, plot(x, fit, type = "n", ylim = ylim, xaxs = "i", yaxs = "i",
                  ylab = "sero-prevalence", xlab = "age (years)", ...))
...
plot_seroprevalences <- function(data, bs = "tp", k = 50, th1 = 200, th2 = 150) {
  data <- mutate(data, positive1 = titer > th1,
                       positive2 = titer > th2)  
    
  gam2(positive1 ~ s(age, bs = bs, k = k), data) |> 
    predict2() |> 
    plot_pred(col = 3)
  
  gam2(positive2 ~ s(age, bs = bs, k = k), data) |> 
    predict2() |> 
    add_ribbon(2)
...
compare_bs <- function(data, th = 200, k = 50, n = 1000) {
  tibble(
    ...
    mutate(model   = map(code, ~ gam2((titer > th) ~ s(age, bs = .x, k = k), data)),
...
dataset |> 
  mutate(across(titer, log10)) |> 
  image_titer_density_age(xaxis = FALSE, xlab = NA, min_titer = log10(5))
add_years_on_top()
par(plt = c(.105, .97, .15 / 2 + (1 - .95) / 2, .5), new = TRUE)
plot_seroprevalences(dataset)
par(opar)
...
seroprevT1T2 <- function(x, th = 200, bs = "tp", k = 50, title = FALSE) {
  x |> 
    ...
    gam2((titer > th) ~ s(age, bs = bs, k = k), data = _) |> 
    predict2() |> 
    plot_pred(col = 2)
