...
```{r}
plot_blank <- function() plot(1, axes = FALSE, ann = FALSE, type = "n")
```
...
```{r include=FALSE, eval=FALSE, fig.width = 7, fig.height = 50 / 3}
tmp <- south |> 
...
```{r include=FALSE, eval=FALSE, fig.width = 7, fig.height = 100 / 3}
tmp <- dataset |> 
  with(table(age_group, province, time_point)) |>
  as.data.frame()
...
opar <- par2(mfcol = c(20, 3), plt = c(.12, .985, .1, .9))
tmp |> 
  group_by(time_point, province) |> 
  group_split() |> 
  walk(~ barplot2(.x, ylim = ylim))
...
#opar <- par2(mfrow = c(20, 4), plt = c(.12, .985, .1, .9))
