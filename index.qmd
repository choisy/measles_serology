...
A function that retrieve the percentage of deviance explained from a GAM model:
...
```{r fig-titers_raw_data, fig.width = 7, fig.height = 5}
dataset |> 
...
models <- tibble(
  ...
  mutate(model   = map(code, ~ gam(log_titer ~ s(age, bs = .x, k = 50),
...
```{r}
predict2_0 <- function(model, x = seq(0, 70, le = 512)) {
  bind_cols(tibble(x  = x),
            as_tibble(predict(model, list(age = x), se.fit = .95)))
}
```
...
```{r}
plot_pred <- function(pred, ylog = TRUE, ylab = "mean titer (IU)", ylim = NULL, ...) {
  with(pred, {
    ...
    polygon(c(x, rev(x)), c(ll, rev(ul)), border = NA, col = adjustcolor(4, .2))
...
```{r fig-gam_titer, fig.width = 7, fig.height = 5}
models |> 
  filter(code == "tp") |> 
  pull(model) |> 
  first() |>
...
models_prev <- tibble(
  smoothing_basis = c("thin plate", "thin plate", "Duchon", "cubic regression",
                      ...
                      "cyclic P-spline", "Gaussian process"),
  code = c("tp", "ts", "ds", "cr", "cs", "cc", "bs", "ps", "cp", "gp")) |> 
  mutate(model   = map(code, ~ gam(positive1 ~ s(age, bs = .x, k = 50), binomial,
                                   data2, method = "REML")),
         REML    = map_dbl(model, get_reml),
         per_dev = map_dbl(model, dev_explained),
         AIC     = map_dbl(model, AIC),
...
```{r fig-gam_seroprev, fig.width = 7, fig.height = 5}
models_prev |> 
  filter(code == "tp") |> 
  pull(model) |> 
  first() |>
...
```{r}
f1 <- with(data2, kde2d(age, log_titer, n = 500))
image(f1)
f2 <- with(data2, kde2d(age, log_titer, n = 500), h = c(width.SJ(age), width.SJ(log_titer)))
image(f2)
```
...
data2 |> 
  filter(age < 20) |> 
  with(kde2d(age, log_titer, n = 500, h = c(width.SJ(age), width.SJ(log_titer)))) |> 
  image()
```

```{r}
min_titer <- min(data2$log_titer)
max_titer <- max(data2$log_titer)
a <- data2 |> 
  group_by(age_group) |> 
  group_map(~ density(.x$log_titer, from = min_titer, to = max_titer)) |> 
  map(~ with(.x, tibble(x, y))) |> 
  map(pad)
```
