...
age_lab <- "age (years)"
col_female <- rgb(247, 114, 184, max = 255)
col_male <- rgb(62, 146, 247, max = 255)
```
...
required <- c("sf", "readxl", "stringr", "stringi", "purrr", "tidyr", "magrittr",
              "RColorBrewer", "lubridate", "mgcv", "forcats", "dplyr")
```
...
```{r}
predict4 <- function(object, newdata, ci = .95, ...) {
  if (! is.data.frame(newdata)) newdata <- as_tibble(newdata)
  bind_cols(newdata, as_tibble(predict2(object, newdata, se = ci, ...)))
} 
```

The following function takes a vector `v` of densities of probabilities as an input and
returns a threshold value above which the values of `v` belong to the `p` confidence
interval:
...
A function that converts the names of a data frame into Latin ASCII:
...
A tuning of the `polygon()` function for densities:
...
A tuning of the `polygon()` function for bands:

```{r}
polygon3 <- function(x, l, u, ...) {
  polygon(c(x, rev(x)), c(l, rev(u)), border = NA, ...)
}
```
...
```{r}
yellow_red <- function(n) hcl.colors(n, "YlOrRd", rev = TRUE)
```

A tuning of the `str_split()` function:

```{r}
str_split2 <- function(...) first(stringr::str_split(...))
```
...
south <- the3files |> 
  ...
  mutate(province = ifelse(is.na(province), hospital, province),
         ...
         across(age, ~ ifelse(.x %% 1 == 0 & .x > 0, .x + .5, .x)),
...
north <- c("02FL Tracking sample round 1 to 6 Final.xlsx",
  ...
  mutate(across(province, convert),
         ...
         unit      = trimws(convert(str_remove_all(age, "\\d"))),
         age       = as.numeric(str_remove_all(age, "\\D")),
         age       = ifelse(unit == "ngay", age / 365,
                            ifelse(unit == "thang", age / 12, age)),
         age = ifelse(`Lab ID` %in% paste(17401:17405), age, age + .5), # age correct
         age_group = make_age_group(age)) |> 
...
dataset <- north |> 
  ...
  mutate(region = ifelse(province %in% unique(north$province), "north", "south"))
```

```{r include = FALSE, eval = FALSE}
writexl::write_xlsx(dataset, "measles_serology_all_Vietnam.xlsx")
```
...
The order of provinces according to latitude:
...
```{r fig-nb_samples_site_timepoint_ageclass, fig.width = 7, fig.height = 100 / 6}
#| fig-cap: "Number of samples collected as a function of age class and site (ordered
#| from south to north from bottom to top) for the three time points (Dec 2019, Dec
#| 2022 and Apr 2024 from left to right). Age classes are 1-year age classes from 0 to
#| 20 and 5-year age classes from 20 to 70 year-old. The horizontal red lines show the
#| targetted number of samples per age class which are 5 for the 1-year age classes
#| below 20 year-old and 10 for the 5-year age classes above 20 year-old."
...
pie_row <- function(x, col = c(col_female, col_male)) {  
...
```{r fig-sex_distribution, fig.width = (4 * .7 * 7 / 3) / 3, fig.height = (.7 * 100 / 3) / 3}
#| fig-cap: "Proportion of female (pink) and male (blue) samples as a function of site
#| (ordered from south to north from bottom to top) for the three time points (Dec
#| 2019, Dec 2022 and Apr 2024 from left to right)."
...
#| fig-cap: "Distribution of titers across all sites and time points. Colors show
#| negative (red), equivocal (orange) and positive (green)."
plot_titers(dataset)
```
...
#| fig-cap: "Distribution of titers across all sites for the 3 time points. Colors show
#| negative (red), equivocal (orange) and positive (green)."
...
image_titer_density_age <- function(data, xaxis = TRUE, xlab = age_lab, n = 1024,
  ...
  data |> 
    filter(age < 70) |>
    mutate(across(age_group, ~ fct_recode(.x, NULL = "[70,100)"))) |> 
...
A function that plots both the raw titer data and the modelled distribution per age
class:

```{r}
plot_titers_age_profile <- function(df, ymin = .9, ymax = 4.3) {
...
Let's plot the distributions of the titers as a function of age for the whole data set:

```{r fig-titer_density_age, fig.height = 2 * eps * figh}
#| fig-cap: "Distribution of titers across all sites and time points for the 20 age
#| classes. Colors show 20 ranges of high density intervals (HDI) from 0 (yellow) to 1
#| (red) by steps of 0.05."
plot_titers_age_profile(dataset)
```

A function that plots the density with a color code according to the high density
interval (HDI):
...
```{r fig-titer_density_HDI}
#| fig-cap: "An example of the distribution of the titers for the 10-11 year-old
#| children together with a color code mapping high density intervals (HDI) from 0
#| (yellow) to 1 (red) by steps of 0.05."
...
The age profiles of titers density in for the north and the south and the 3 time
points:

```{r fig-titer_age_profile_time_region}
#| fig-cap: "Distributions of titers across regions and time points for the 20 age
#| classes and the 3 time points. Colors show 20 ranges of high density intervals (HDI)
#| from 0 (yellow) to 1 (red) by steps of 0.05."
#| fig-subcap: ["North Vietnam, T1 (Dec 2019)",
#|              "North Vietnam, T2 (Aug 2022)",
#|              "South Vietnam, T1 (Dec 2019)",
#|              "South Vietnam, T2 (Dec 2022)",
#|              "South Vietnam, T3 (Apr 2024)"]

dataset |> 
  mutate(across(titer, log10)) |>
  group_by(region, time_point) |> 
  group_walk(~ image_titer_density_age(.x, min_titer = .9, max_titer = 3.4))
```

### Titers per age class and time point

A function that plots the decline of maternal antibody titers:

```{r}
plot_maternal_immunity <- function(df, remove_age0 = TRUE, k = -1, n = 512, ci = .95,
                                   xlim = c(0, 12), ylim = c(.3, 3.4)) {
  tmp <- df |> 
    filter(age_group == "[0,1)") |> 
    mutate(across(titer, log10),
           across(age, ~ 12 * .x),
           across(sex, ~ as.numeric(as.factor(.x))))
  
  if (remove_age0) tmp <- filter_out(tmp, age == 0)
  ...
  preds <- tmp |>
    gam(titer ~ s(age, k = k), data = _) |> 
    predict4(list(age = seq(0, 12, le = n)), ci = ci)
    
  plot(1, type = "n", xlim = xlim, ylim = ylim, yaxt = "n",
       xlab = "age (months)", ylab = titer_lab)
  
  with(preds, {
    polygon3(age, ll, ul, col = "lightgrey")
    lines(age, fit)
  })
  with(tmp, points(age, titer, col = c(col_female, col_male)[sex], pch = 19))
  abline(h = log10(c(150, 200)), col = 2:3, lty = 2)
  abline(v = 9, lty = 2)
  logaxis(2, 0:5)
}
```
...
```{r}
dataset |> 
  filter(region == "south", time_point == "T1") |> 
  plot_maternal_immunity(k = 8)
```

```{r}
dataset |> 
  filter(region == "south", time_point == "T2") |> 
  plot_maternal_immunity()
```

```{r}
dataset |> 
  filter(region == "south", time_point == "T3") |> 
  plot_maternal_immunity()
```


```{r}
plot_titers_age <- function(df, k = -1, col = "lightgrey", ylim = c(2, 3.5), n = 512) {
  df |> 
    mutate(across(age, ~ ifelse(.x == 0, .5, .x)),
           across(titer, log10)) |> 
    gam(titer ~ s(age, k = k), data = _) |> 
    predict4(list(age = seq(0, 70, le = n))) |> 
    with({
      plot(age, fit, type = "n", ylim = ylim, yaxt = "n",
           xlab = age_lab, ylab = titer_lab)
      polygon3(age, ll, ul, col = col)
      lines(age, fit)
    })
  abline(h = log10(c(150, 200)), col = 2:3, lty = 2)
  logaxis(2, 0:5)
...
```{r}
dataset |> 
  filter(region == "north", time_point == "T1") |> 
  plot_titers_age(k = 50)
```

```{r}
dataset |> 
  filter(region == "north", time_point == "T2") |> 
  plot_titers_age(k = 50)
```
...
```{r include = FALSE}
if (file_exists("datasetT1.rds")) {
  datasetT1 <- readRDScache("datasetT1.rds")
} else {
  datasetT1 <- dataset |>
    filter(time_point == "T1") |> 
    mutate(across(titer, log10)) |> 
    agglomerate() |> 
    mutate(across(titer, ~ 10^.x))
  saveRDScache(datasetT1, "datasetT1.rds")
}
```

Let's plot the age profiles for the identified clusters of provinces:

```{r}
datasetT1 |> 
  mutate(across(titer, log10)) |> 
  group_by(province) |> 
  group_walk(~ image_titer_density_age(.x, min_titer = .9, max_titer = 4.3, n = 2048))
```

```{r}
data <- datasetT1 |> 
  mutate(across(titer, log10)) |> 
  filter(province == "Thai Binh | Lao Cai | Ha Noi | Ha Tinh") |> 
  image_titer_density_age(min_titer = .9, max_titer = 4.3, n = 2048)
```


The list of province for each of the plots above:

```{r}
unique(datasetT1$province)
```
...
**TO DO:** rewrite it as a function of `predict4()`
...
where `pred` is an output of the `predict4()` function. A wrapper around the
...
```{r}
seroprevT1 <- function(x, th = 200, bs = "tp", k = 50, title = FALSE) {
  x |> 
    filter(time_point == "T1") |> 
